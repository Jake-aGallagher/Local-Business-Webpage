...
const ReservationForm = () => {
  ...
  const makeReservation = (event) => {
    ...
    alert(
      "Please check the console to see the Form Data in JSON format ready for dispatch to a backend."
    );
    console.log(
      "{\n",
      "{ Boodking Date:",
      event.target.bookingDate.value,
      "}\n",
      "{ Time:",
      event.target.time.value,
      "}\n",
      "{ Party size:",
      event.target.persons.value,
      "}\n",
      "{ Name:",
  ...
  return (
    <div className={classes.form}>
      ...
      <form className={classes.formInputs} onSubmit={makeReservation}>
        <input
          id="bookingDate"
          /* Allows a placeholder when not in focus but changes to date form input when being used */
          type={dateType}
